...
describe('TopBar', ()=>{
    ...
    describe('interactions', () => {
        ...
        it('removes show class to drop down menu when clicking logout' ,async () => {
            const { queryByText, findByTestId} = setup(loggedInState);
            const displayName = queryByText('display1');
            fireEvent.click(displayName);
            fireEvent.click(queryByText('Logout'));
            store.dispatch(authActions.loginSucces(loggedInState))
            const dropDownMenu = await findByTestId('drop-down-menu');
            expect(dropDownMenu).not.toHaveClass('show');
        });
